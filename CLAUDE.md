...
- **Find every other document that states the same thing, and change it too.** `CLAUDE.md`, `CONTRIBUTING.md`, `README.md`, `.claude/README.md` and the skills deliberately say the same things for different readers, and a change to the gate, the release procedure, or the five rules touches four or five files at once. `just doc-links` is the link-shaped half of this and catches a broken hyperlink; it does not catch two documents disagreeing about a fact between them that isn't a link — a `1.4` that should read `1.5`, a procedure restated as numbered steps in a second document and not reordered with the first, an index cell summarizing a fact its own detailed entry no longer says. That is judgment, not syntax, which is why this stayed a checklist line in `.claude/skills/review-response/LESSONS.md` through five misses across two pull requests before landing here — including, fittingly, two branches cut from the same commit each finding their own drift and missing the other's, which is exactly the failure mode the rule describes. See `.claude/skills/review-response/RETIRED.md` for the evidence.
