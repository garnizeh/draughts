...
**A promoted rule missed again is recorded differently depending on where it landed.** If it graduated to `CLAUDE.md`, a future review finding the same failure is a **standing** miss: open a fresh entry for it (or add to one already there) under its section here, with **origin: none** — the rule was not learned here the second time, `CLAUDE.md` already stated it, and `missed` is what says the permanent line did not stop the mistake either. If it graduated to a script instead, a gap the script misses is a bug in the script, not a miss on a retired rule: fix it and extend the script's own test, the same as any other code defect — the whole point of mechanizing a property was to stop needing a counter for it.
...
*Find every other document that states the same thing, and change it too* graduated from here to `CLAUDE.md` on 2026-09-03, at `missed ×5`. See [RETIRED.md](RETIRED.md) for the evidence — it stays the first place to look when a documentation finding feels like a repeat.
...
- **Do not require something the platform cannot do — and that includes every command in a block someone will paste.** An impossible step is not a high standard, it is a step everyone learns to skip. **The miss taught the second half: a skipped step is not skipped into nothing, it is skipped into a substitute the reader picks unaided, and the substitute is chosen at the worst possible moment.** So a step that always fails is more dangerous than one that is merely useless, and *explaining* underneath the block why it always fails does not fix it — that was the exact shape of the miss. If the safe command cannot succeed here, the block gets the one that can, with the guard rebuilt as something that is true: not the proxy the tool happens to offer, the property the proxy was standing in for. <sub>**origin** #99 (2026-09-02, the PR template demanded a reply "on its own thread" for outside-diff findings, which have no thread and no comment id to reply on) · **missed ×1** #101 (2026-09-03, the new post-merge sequence instructed `git branch -d` one bullet above a paragraph explaining that squash-only merges make `-d` refuse every time in this repository; a reviewer caught it) · **saved ×0**. Hit rate 0/1. The miss bought the rewrite above rather than a promotion, and it is worth noticing *how* it failed: the author knew the command could not succeed, wrote that down, and still left it in the block — so the rule did not need to be believed, it needed to be applied to a code fence rather than to a checklist. The fix was `git diff --quiet main "$BRANCH" && git branch -D "$BRANCH"`, where the tree comparison is strictly stronger than the ancestry test `-d` performs; squash breaks the proxy, not the property.</sub>
  ...
  <!-- scope: paths=["**/*.md", ".claude/**", ".github/*TEMPLATE*"] change_kinds=[rule_or_policy] -->
- **A permission stated in one sentence and a stricter assumption in a nearby one have to be reconciled, not left to coexist.** "Commit as one or several, it does not matter" and "reply citing its commit SHA" sat four lines apart, and the second sentence quietly assumed the first had chosen one — a reader who actually split a round into two commits would have nothing to cite. This is the single-document version of *find every other document that states the same thing*: two sentences in the same paragraph are still two places deciding the same question, and proximity does not make them agree on its own. <sub>**origin** #103 (2026-09-03, `review-response/SKILL.md`'s phase 5 permitted several commits per push and then said "its commit SHA" as if there could only be one; found on the PR that had just finished reordering the same phase, by a reviewer reading the two sentences together) · **missed ×0** · **saved ×0**. Fixed by keeping the permission and naming what "its" refers to: the tip of the push, whatever number of commits got there.</sub>
  <!-- scope: paths=["**/*.md", ".claude/**", ".github/*TEMPLATE*"] change_kinds=[rule_or_policy] -->
...
## If a procedure ends with a gate or a verification step

- **A gate runs before the action it is supposed to gate, never as a report filed after.** `just pre-pr` sitting after "push" and "reply" in a numbered procedure reads as a closing formality, and that is exactly backwards: a failure it would have caught is now a failure already pushed and already replied about, fixable only by a second push — which, inside `review-response`, is the identical problem that phase 5 exists to prevent, one step further out. The fix is always to move the gate earlier, never to make its failure cheaper to recover from after the fact. <sub>**origin** #103 (2026-09-03, review-response's own step order — reply, harvest, push, then `just pre-pr` last — put the full local gate after the push it should have protected; caught by the user reading the transcript, not by CodeRabbit, while a second PR against this same skill was already reordering push relative to harvest) · **missed ×0** · **saved ×0**.</sub>
  <!-- scope: paths=[".claude/commands/*.md", ".claude/skills/*/SKILL.md"] change_kinds=[rule_or_policy, documentation] -->

## If you are harvesting a finding on a rule that already graduated

- **A promotion defines where a rule goes; it does not define what happens the next time the rule fails.** The mechanics above cover earning, missing, saving, and the two roads out — a script at `missed ×2`, `CLAUDE.md` at `missed ×5` for a judgment rule — and stopped there, leaving no answer for a review that finds a `CLAUDE.md` rule violated again, or a gap in a script that already graduated. Both cases are now stated in the Promotion section: a `CLAUDE.md` miss is recorded here as a **standing** entry (origin: none, the rule was not learned here this time), and a script's gap is a bug in the script, fixed and tested like any other code, never a `missed` entry on a rule that no longer lives here. <sub>**origin** #103 (2026-09-03, found while harvesting the review that promoted *find every other document* to `CLAUDE.md` in the same PR — the promotion section said what crosses the threshold and where it goes, and nothing about what a later failure of the same rule should be called) · **missed ×0** · **saved ×0**.</sub>
  <!-- scope: paths=[".claude/skills/review-response/LESSONS.md", ".claude/commands/respond.md"] change_kinds=[rule_or_policy, documentation] -->
