...
|---|---|---|---|
| Find every other document that states the same thing | [documentation](#if-you-changed-documentation) | 4/8 | This ledger's own index and detailed entry for the row above drifted from each other: the index named "a subsection list that gained an entry" as the third drifted document, which matches nothing in the detailed entry — the real third item is a `GameState` field missing from two declarations in one file. **Carries a false-positive note — read it before hunting** |
