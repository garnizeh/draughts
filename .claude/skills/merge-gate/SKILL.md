...
- **`-d`, not `-D`.** `-d` refuses to delete a branch whose commits are not in `main`, which is the one guard standing between a mistyped branch name and lost work. **Here it will refuse every time**: squash is this repository's only permitted merge method (`mergeCommitAllowed` and `rebaseMergeAllowed` are both false), so the commit on `main` is never the branch's commit and the ancestry test cannot pass. That refusal is the expected outcome, not evidence of a problem — but the guard is still worth having, because the check that replaces it is a positive one rather than a shrug: `git diff main BRANCH` (two dots — same trees, not same history) prints nothing, proving the content landed, and only then `git branch -D BRANCH`.
- **The remote half is already done.** `deleteBranchOnMerge` is on for this repository, so `git push origin --delete` will normally fail with "remote ref does not exist" — that is success arriving early, not an error to chase. Run it when the branch is still listed, skip it when `git fetch --prune` already took the tracking ref away. The setting is a repository option someone can turn off, which is why the line stays in the sequence rather than being dropped as redundant.
